...
def overlapping_area(detection_1, detection_2, show = False):
    '''
    Calculate the coverage size of two detection areas，detection：(x, y, pred_prob, width, height, area)
    '''
    ...
    # top left and bottom right of detection_2
    x2_tl = detection_2[0]
    y2_tl = detection_2[1]
    ...
    # 1. Calculate overlapping area_1
    # total_area = area_1 + area_2 - overlap_area
    # return overlap_area / float(total_area)

    # 2. Calculate overlapping area_2
    area = area_1
    if area_1 < area_2:
        area = area_2
...
def nms(detections, threshold=0.1):
    ...
    1. The maximum confidence value comes first
    2. Maximum area first
    Non maximum suppression reduces overlapping areas, detection:(x, y, pred_prob, width, height)
    '''
    if len(detections) == 0:
        return []
